...
def check_for_degenerate_vtts(vtt_dir, audio_dir='', file_type='vtt', language='de',
                              ...
                              p_connection=None, p_cursor=None, simulate=False):
    ...
    for file in tqdm(files):
        ...
        if audio_dir != '':
            ...
            input_audio = os.path.join(audio_dir, filename.rsplit('.', 1)[0]) + '.flac'
    ...
    if len(degen_files) > 0:
        ...
        with open(possibly_corrupted_outfile, 'w') as outfile:
            for file, degen_num_lines in degen_files:
                assert(file is not None)
                assert(file != '')
                assert(file.endswith(f".{file_type}"))

                print('file:', file, 'len distinct lines very small:', degen_num_lines)
                outfile.write(file + '\n')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--simulate', help='Simulate the operation without moving files or changing anything in the database', action='store_true')
    ...
    check_for_degenerate_vtts(args.vtt_dir, args.audio_dir, file_type=args.file_type, language=args.language, p_connection=p_connection, p_cursor=p_cursor, simulate=args.simulate)
